...
const STARRED = [
  { ticker: "GW150914", name: "GW150914 — BBH (Hanford+Livingston)", sector: "BBH", price: 28.6, mu: 0.001, sigma: 0.04 },
  { ticker: "GW170817", name: "GW170817 — BNS (H+L+V)", sector: "BNS", price: 1.188, mu: 0, sigma: 0.02 },
  { ticker: "GW190521", name: "GW190521 — IMBH candidate (H+L+V)", sector: "IMBH", price: 65.1, mu: 0.005, sigma: 0.18 },
  { ticker: "GW200115", name: "GW200115 — NSBH (H+L+V)", sector: "NSBH", price: 5.9, mu: 0.002, sigma: 0.1 },
  { ticker: "GW230529", name: "GW230529 — mass-gap (H+L)", sector: "NSBH/gap", price: 3.6, mu: 0.003, sigma: 0.12 },
  { ticker: "GW231123", name: "GW231123 — high-mass BBH (H+L+V)", sector: "BBH", price: 95.4, mu: 0.006, sigma: 0.22 },
  { ticker: "GW170814", name: "GW170814 — BBH (H+L+V triple)", sector: "BBH", price: 24.1, mu: 0.001, sigma: 0.05 },
  { ticker: "GW170608", name: "GW170608 — light BBH (H+L)", sector: "BBH", price: 7.9, mu: 0.001, sigma: 0.06 },
  { ticker: "GW190412", name: "GW190412 — asymmetric BBH (H+L+V)", sector: "BBH", price: 13.2, mu: 0.002, sigma: 0.08 },
  { ticker: "GW190814", name: "GW190814 — mass-gap (H+L+V)", sector: "NSBH/gap", price: 6.1, mu: 0.003, sigma: 0.1 },
  { ticker: "GW240817", name: "GW240817 — O4 candidate (H+L+V+K)", sector: "BBH", price: 32.4, mu: 0.004, sigma: 0.15 },
  { ticker: "GW250121", name: "GW250121 — eccentric (H+L+V)", sector: "BBH", price: 41.7, mu: 0.005, sigma: 0.2 },
];
  ...
  "GW150914": [
    {
      ...
      "author": "@ligo_caltech",
      "handle": "1h",
      "text": "$GW150914 first direct GW detection (2015). Chirp mass 28.6 M☉, masses 36+29 M☉ BBH at 410 Mpc."
    },
    {
      "sent": "pos",
      "author": "@nature_phys",
      "handle": "1d",
      "text": "$GW150914 SNR 24, 5.1σ. Defining event of the field — every subsequent BBH compared to this template."
    }
  ],
  "GW170817": [
    {
      "sent": "pos",
      "author": "@multimessenger",
      "handle": "2h",
      "text": "$GW170817 BNS — kilonova SSS17a co-detected across EM spectrum. Multi-messenger astronomy era opens."
    },
    {
      "sent": "pos",
      "author": "@gw_papers",
      "handle": "9h",
      "text": "$GW170817 tidal deformability constraints rule out stiffest NS EoS. Neutron-star matter physics breakthrough."
    }
  ],
  "GW190521": [
    {
      ...
      "author": "@imbh_search",
      "handle": "3h",
      "text": "$GW190521 IMBH-mass-gap candidate. Total mass 150 M☉ — likely from hierarchical merger or PISN gap."
    },
    {
      "sent": "neg",
      "author": "@bayes_pe",
      "handle": "1d",
      "text": "$GW190521 eccentric-precessing template debate ongoing. Posterior multimodal — story isn't settled."
    }
  ],
  "GW200115": [
    {
      ...
      "author": "@nsbh_team",
      "handle": "4h",
      "text": "$GW200115 confirmed NSBH — first unambiguous detection. 5.7 M☉ BH + 1.5 M☉ NS."
    }
  ],
  "GW230529": [
    {
      ...
      "author": "@ligo_o4",
      "handle": "2h",
      "text": "$GW230529 lower-mass-gap candidate. 2.5–4.5 M☉ object challenges canonical BH/NS distinction."
    }
  ],
  "GW231123": [
    {
      "sent": "neu",
      "author": "@bbh_high_mass",
      "handle": "5h",
      "text": "$GW231123 high-mass BBH — total mass 190 M☉. Strong precession signature in posterior."
    }
  ],
  "GW170814": [
    {
      ...
      "author": "@ligo_caltech",
      "handle": "6h",
      "text": "$GW170814 first 3-detector BBH detection. Sky localization shrunk by 10x — Virgo earns its keep."
    }
  ],
  "GW170608": [
    {
      ...
      "author": "@bbh_pop",
      "handle": "1d",
      "text": "$GW170608 lightest BBH at the time. Constrains low-mass end of BH mass function."
    }
  ],
  "GW190412": [
    {
      ...
      "author": "@asymmetric",
      "handle": "3h",
      "text": "$GW190412 asymmetric BBH — q ≈ 0.28. Higher-mode signal detected. Spin precession evidence."
    }
  ],
  "GW190814": [
    {
      ...
      "author": "@mass_gap",
      "handle": "10h",
      "text": "$GW190814 most asymmetric BBH/NSBH so far — secondary 2.6 M☉ either heaviest NS or lightest BH known."
    }
  ],
  "GW240817": [
    {
      ...
      "author": "@o4_collab",
      "handle": "1h",
      "text": "$GW240817 — O4 BBH candidate at distance 1.2 Gpc. Spin posterior favors aligned."
    }
  ],
  "GW250121": [
    {
      ...
      "author": "@eccentric_gw",
      "handle": "2h",
      "text": "$GW250121 eccentric BBH candidate. Eccentricity at 10Hz reference 0.2 — first robust eccentric detection."
    },
    {
      "sent": "neu",
      "author": "@waveform_dev",
      "handle": "1d",
      "text": "$GW250121 waveform systematics non-trivial — eccentric templates still maturing."
    }
  ]
};
...
const STRATEGIES = [
  {
    ...
    apply: (ctx) => {
      ...
      if (s20 == null || s50 == null) return { signal: "MARGINAL", detail: "Not enough history for 50-day MA." };
      if (s20 > s50 * 1.005) return { signal: "DETECTION",  detail: `20-day (${s20.toFixed(2)}) above 50-day (${s50.toFixed(2)}) — bullish trend.` };
      if (s20 < s50 * 0.995) return { signal: "GLITCH", detail: `20-day (${s20.toFixed(2)}) below 50-day (${s50.toFixed(2)}) — bearish trend.` };
      return { signal: "MARGINAL", detail: `20-day ≈ 50-day — consolidation.` };
    },
  },
  {
    id: "rsi_mr",
    ...
    apply: (ctx) => {
      ...
      if (r < 30) return { signal: "DETECTION",  detail: `RSI ${r.toFixed(1)} — oversold, mean-revert up.` };
      if (r > 70) return { signal: "GLITCH", detail: `RSI ${r.toFixed(1)} — overbought, mean-revert down.` };
      return { signal: "MARGINAL", detail: `RSI ${r.toFixed(1)} — mid-range, no edge.` };
    },
  },
  {
    id: "momentum",
    name: "Pipeline Momentum",
    desc: "PyCBC vs GstLAL",
    apply: (ctx) => {
      const h = ctx.history;
      if (h.length < 21) return { signal: "MARGINAL", detail: "Not enough data." };
      const r20 = (h[h.length - 1] - h[h.length - 21]) / h[h.length - 21];
      if (r20 > 0.05)  return { signal: "DETECTION",  detail: `+${(r20 * 100).toFixed(1)}% past 20d — ride the trend.` };
      if (r20 < -0.05) return { signal: "GLITCH", detail: `${(r20 * 100).toFixed(1)}% past 20d — negative momo.` };
      return { signal: "MARGINAL", detail: `${(r20 * 100).toFixed(1)}% past 20d — flat.` };
    },
  },
  {
    id: "mc_asymmetry",
    ...
    desc: "high-mass tail",
    apply: (ctx) => {
      if (!ctx.mcSummary) return { signal: "MARGINAL", detail: "Run a simulation." };
      ...
      if (ratio > 1.3 && expectedReturn > 0.01) return { signal: "DETECTION", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      if (ratio < 0.8 && expectedReturn < -0.01) return { signal: "GLITCH", detail: `Up/down ratio ${ratio.toFixed(2)}x, E[R] ${(expectedReturn * 100).toFixed(1)}%, Pr(up) ${(probUp * 100).toFixed(0)}%.` };
      return { signal: "MARGINAL", detail: `Up/down ratio ${ratio.toFixed(2)}x — no clear edge.` };
    },
  },
  {
    id: "x_sentiment",
    name: "Collaboration Notes",
    desc: "net DCC sentiment",
    apply: (ctx) => {
      const s = ctx.sentiment;
      if (s.score > 0.25)  return { signal: "DETECTION",  detail: `Net +${(s.score * 100).toFixed(0)} — crowd bullish.` };
      if (s.score < -0.15) return { signal: "GLITCH", detail: `Net ${(s.score * 100).toFixed(0)} — crowd bearish.` };
      return { signal: "MARGINAL", detail: `Net ${(s.score * 100).toFixed(0)} — mixed crowd.` };
    },
  },
];

function signalScore(signal) { return signal === "DETECTION" ? 1 : signal === "GLITCH" ? -1 : 0; }

function combineSignals(results) {
  if (!results.length) return { signal: "MARGINAL", detail: "Select at least one strategy." };
  const avg = results.reduce((a, r) => a + signalScore(r.signal), 0) / results.length;
  if (avg > 0.35)  return { signal: "DETECTION",    detail: "Majority bullish." };
  if (avg < -0.35) return { signal: "GLITCH",   detail: "Majority bearish." };
  return { signal: "MARGINAL", detail: "Mixed strategy signals." };
}
...
function renderTickerSelect() {
  ...
  el.addEventListener("change", (e) => {
    ...
    renderExtra3Panel();
    renderExtra4Panel();
    announce(`Selected ${state.selectedTicker}. Event detail loaded.`);
  });
}
...
function signalBadge(signal, opts = {}) {
  const cls = signal === "DETECTION" ? "badge-buy" : signal === "GLITCH" ? "badge-sell" : "badge-unsure";
  const icon = signal === "DETECTION" ? "▲" : signal === "GLITCH" ? "▼" : "◆";
...
const kpiDefs = () => {
  ...
  return [
    { id: "kpi-value",  label: "Portfolio value", value: k.value,           fmt: (v) => dollarFmt(v), mod: "accent",                              sub: `${state.stocks.length} events · equal weight` },
...
function renderKPIs() {
  ...
  // Right rail
  const buys = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "DETECTION").length;
  const sells = state.stocks.filter((x) => getCombinedSignalForStock(x).signal === "GLITCH").length;
  ...
  setText("#rail-buys-sub", `${state.stocks.length} events total`);
...
function renderStocksTable() {
  ...
  body.innerHTML = rows.map((s) => {
    ...
    const dotCls = s.combinedSignal.signal === "DETECTION" ? "buy" : s.combinedSignal.signal === "GLITCH" ? "sell" : "unsure";
...
function renderDetail(onComplete) {
  ...
  $("#detail-subtitle").textContent = `Source class: ${stock.sector} · Chirp mass ${priceFmt(stock.price)} (${pctFmt(stock.change)}) · Drift μ=${(stock.mu * 100).toFixed(1)}%, vol σ=${(stock.sigma * 100).toFixed(1)}%.`;
...
function renderSentiment(stock) {
  ...
  const posts = s.posts.length ? s.posts : [{ sent: "neu", author: "@market_data", handle: "now", text: "No recent public chatter for this event." }];
...
function renderSummary() {
  ...
  const buy = all.filter((x) => x === "DETECTION").length;
  const sell = all.filter((x) => x === "GLITCH").length;
  const uns = all.filter((x) => x === "MARGINAL").length;
  const winner = buy > sell && buy > uns ? "DETECTION" : sell > buy && sell > uns ? "GLITCH" : "MARGINAL";
  const agg = `${buy} DETECTION · ${uns} MARGINAL · ${sell} GLITCH across ${all.length} events.`;
...
function spawnParticles(origin, kind) {
  ...
  if (!layer) return;
  const count = kind === "DETECTION" ? 18 : 14;
  for (let i = 0; i < count; i++) {
    const p = document.createElement("div");
    p.className = "fx-particle" + (kind === "GLITCH" ? " sell" : "");
    p.style.left = origin.x + "px";
    p.style.top = origin.y + "px";
    layer.appendChild(p);
    const angle = kind === "DETECTION"
      ? (Math.random() * Math.PI * 2)
      : (Math.PI / 2 + (Math.random() - 0.5) * 0.7);
    const speed = kind === "DETECTION" ? 40 + Math.random() * 80 : 20 + Math.random() * 40;
    const dx = Math.cos(angle) * speed;
    const dy = Math.sin(angle) * speed + (kind === "GLITCH" ? 70 + Math.random() * 40 : 0);
...
function particlesForSignalFlip(row, signal) {
  if (signal !== "DETECTION" && signal !== "GLITCH") return;
  const badge = row.querySelector(".badge");
  if (!badge) return;
...
function runBootSequence(onComplete) {
  ...
  const alreadySeen = sessionStorage.getItem("gwt.booted") === "1";
  ...
  const finish = () => {
    ...
    sessionStorage.setItem("gwt.booted", "1");
  ...
  const lines = [
    "> GWT-TERMINAL v4.7  (c) gravitationalwaveterminal",
    "> booting kernel ........................ <span class='ok'>OK</span>",
    "> mounting event watchlist ........... <span class='ok'>OK</span>",
    `> loading ${STARRED.length} events .................... <span class='ok'>OK</span>`,
    "> initializing monte carlo engine (GBM) .. <span class='ok'>OK</span>",
    "> hooking GWT sentiment stream ........ <span class='ok'>OK</span>",
...
function renderExtraPanel() {
  ...
  const ev = state.stocks.find((s) => s.ticker === state.selectedTicker) || state.stocks[0];
  const w = 720, h = 320, padL = 64, padR = 12, padT = 18, padB = 32;
  const inW = w - padL - padR, inH = h - padT - padB;
  // Grid: 64 time × 32 freq (display); sr-only fallback 16 × 8
  const nT = 64, nF = 32, srT = 16, srF = 8;
  const tMin = -0.3, tMax = 0.05; // seconds around merger
  const fMin = 20, fMax = 1024;
  const tStep = (tMax - tMin) / nT;
  // Chirp curve: f(t) = (1/8π) * (5 / (256 * t_c-t))^(3/8) * (G M / c^3)^(-5/8) — use proxy
  const M = ev.price;
  function chirpF(t) {
    if (t >= tMax) return fMax;
    const tau = Math.max(1e-4, tMax - t);
    return Math.min(fMax, 134 * Math.pow(M / 28.6, -5 / 8) * Math.pow(tau, -3 / 8));
  }
  // Strain magnitude grid
  function cellMag(ti, fi) {
    const t = tMin + ti * tStep;
    const f = fMin * Math.pow(fMax / fMin, fi / (nF - 1));
    const cf = chirpF(t);
    const sigma = Math.max(0.3, 0.5 - (t / tMax) * 0.2) + ev.sigma * 0.5;
    const dist = Math.abs(Math.log(f / cf));
  ...
  for (let ti = 0; ti < nT; ti++) {
    for (let fi = 0; fi < nF; fi++) {
      const m = cellMag(ti, fi);
      if (m > peakMag) { peakMag = m; peakT = ti; peakF = fi; }
      const cw = inW / nT, ch = inH / nF;
      const x = padL + ti * cw;
      const y = padT + (nF - 1 - fi) * ch;
      const a = (0.05 + 0.95 * m).toFixed(3);
      cells += `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${(cw + 0.5).toFixed(1)}" height="${(ch + 0.5).toFixed(1)}" fill="#ff66aa" fill-opacity="${a}"/>`;
    }
  }
  // Chirp curve overlay
  let chirpPath = "";
  for (let ti = 0; ti < nT; ti++) {
    const t = tMin + ti * tStep;
    const f = chirpF(t);
    const x = padL + ti * (inW / nT);
    const y = padT + (1 - Math.log(f / fMin) / Math.log(fMax / fMin)) * inH;
    chirpPath += (ti === 0 ? "M" : "L") + x.toFixed(1) + "," + y.toFixed(1) + " ";
  }
  // Axes
  const fLabels = [20, 64, 256, 1024];
  const tLabels = [-0.3, -0.2, -0.1, 0];
  const peakTSec = (tMin + peakT * tStep).toFixed(3);
  const peakFHz = (fMin * Math.pow(fMax / fMin, peakF / (nF - 1))).toFixed(0);
  const fStart = chirpF(tMin).toFixed(0);
  const fEnd = chirpF(tMax).toFixed(0);
  const ariaLabel = `Q-transform spectrogram for ${ev.ticker}. Peak strain at time ${peakTSec} seconds, frequency ${peakFHz} hertz. Chirp sweeps ${fStart} to ${fEnd} hertz over ${(tMax - tMin).toFixed(2)} seconds.`;
  container.innerHTML = `
    <svg viewBox="0 0 ${w} ${h}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="${ariaLabel}">
      <rect x="${padL}" y="${padT}" width="${inW}" height="${inH}" fill="#0a0b0e"/>
      ${cells}
      <path d="${chirpPath}" fill="none" stroke="#fff" stroke-width="1.5" stroke-dasharray="4 3"/>
      ${fLabels.map(f => `<text x="${padL - 6}" y="${padT + (1 - Math.log(f / fMin) / Math.log(fMax / fMin)) * inH + 3}" text-anchor="end" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">${f} Hz</text>`).join("")}
      ${tLabels.map(t => `<text x="${padL + ((t - tMin) / (tMax - tMin)) * inW}" y="${h - 14}" text-anchor="middle" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">${t.toFixed(2)} s</text>`).join("")}
      <text x="${padL}" y="${padT - 4}" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">CHIRP TRACK (white dash) · STRAIN MAGNITUDE (heat)</text>
    </svg>
  `;
  const tk = document.getElementById("extra-ticker"); if (tk) tk.textContent = ev.ticker;
  const sub = document.getElementById("extra-subtitle");
  if (sub) sub.textContent = `${ev.name} · chirp mass ${ev.price.toFixed(2)} M\u2609 · peak strain at t=${peakTSec}s, f=${peakFHz}Hz · chirp ${fStart} → ${fEnd} Hz.`;
  // sr-only binned 16x8 table
  const body = document.getElementById("extra-data-body");
  if (body) {
    let html = "";
    for (let ti = 0; ti < srT; ti++) for (let fi = 0; fi < srF; fi++) {
      const t = tMin + (ti / srT) * (tMax - tMin);
      const f = fMin * Math.pow(fMax / fMin, fi / (srF - 1));
      const m = cellMag(Math.floor((ti / srT) * nT), Math.floor((fi / srF) * nF));
      html += `<tr><td>${t.toFixed(2)}</td><td>${f.toFixed(0)}</td><td>${m.toExponential(2)}</td></tr>`;
    }
    body.innerHTML = html;
  }
}
...
function renderExtra2Panel() {
  ...
  const ev = state.stocks.find(s => s.ticker === state.selectedTicker) || state.stocks[0];
  const w = 720, h = 540;
  // 4×4 corner plot: M (chirp), q (mass ratio), chi_eff (spin), DL (distance)
  const params = ["M", "q", "χ_eff", "D_L"];
  const med = [ev.price, 0.78, 0.05, 410 + ev.sigma * 100];
  const lo = [med[0] * 0.92, 0.62, -0.15, med[3] * 0.78];
  const hi = [med[0] * 1.10, 0.95, 0.22, med[3] * 1.25];
  const units = ["M⊙", "", "", "Mpc"];
  const N = 4;
  const tile = 110, pad = 50;
  const totW = pad + N * tile + 20;
  let panels = "";
  for (let i = 0; i < N; i++) {
    for (let j = 0; j <= i; j++) {
      const x0 = pad + j * tile, y0 = 20 + i * tile;
      if (i === j) {
        // Diagonal: marginal histogram (synth Gaussian)
        let path = "M0,80 ";
        for (let k = 0; k <= 30; k++) {
          const u = k / 30;
          const v = Math.exp(-((u - 0.5) * (u - 0.5)) / 0.04);
          path += "L" + (u * (tile - 10)).toFixed(1) + "," + (80 - v * 70).toFixed(1) + " ";
        }
        path += "L" + (tile - 10) + ",80 Z";
        panels += `<g transform="translate(${x0},${y0})"><rect width="${tile - 4}" height="${tile - 4}" fill="#0a0b0e" stroke="#191c23"/>
          <path d="${path}" fill="#ff66aa" fill-opacity="0.35" stroke="#ff66aa" stroke-width="1"/>
          <text x="${tile / 2}" y="14" text-anchor="middle" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${params[i]}</text>
          <text x="${tile / 2}" y="${tile - 12}" text-anchor="middle" fill="#ff66aa" font-size="9" font-family="JetBrains Mono, monospace">${typeof med[i] === "number" && med[i] < 10 ? med[i].toFixed(2) : Math.round(med[i])}</text>
        </g>`;
      } else {
        // Off-diagonal: 2D credible region (ellipse)
        const seed = (i * 7 + j * 13) % 11;
        const rotDeg = (seed - 5) * 6;
        panels += `<g transform="translate(${x0},${y0})"><rect width="${tile - 4}" height="${tile - 4}" fill="#0a0b0e" stroke="#191c23"/>
          <ellipse cx="${(tile - 4) / 2}" cy="${(tile - 4) / 2}" rx="${(tile - 4) * 0.32}" ry="${(tile - 4) * 0.20}" fill="#ff66aa" fill-opacity="0.15" stroke="#ff66aa" stroke-width="1" transform="rotate(${rotDeg} ${(tile - 4) / 2} ${(tile - 4) / 2})"/>
          <ellipse cx="${(tile - 4) / 2}" cy="${(tile - 4) / 2}" rx="${(tile - 4) * 0.18}" ry="${(tile - 4) * 0.10}" fill="#ff66aa" fill-opacity="0.35" transform="rotate(${rotDeg} ${(tile - 4) / 2} ${(tile - 4) / 2})"/>
          <circle cx="${(tile - 4) / 2}" cy="${(tile - 4) / 2}" r="2.5" fill="#ff9cc3"/>
        </g>`;
      }
    }
    // Right-side labels
    if (i === N - 1) {
      for (let j = 0; j < N; j++) {
        panels += `<text x="${pad + j * tile + tile / 2}" y="${20 + N * tile + 14}" text-anchor="middle" fill="#d6dce4" font-size="10" font-family="JetBrains Mono, monospace">${params[j]}</text>`;
      }
    }
  }
  const fullH = 20 + N * tile + 40;
  container.innerHTML = `<svg viewBox="0 0 ${totW} ${fullH}" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Posterior corner plot for ${ev.ticker}. Medians: M=${typeof med[0] === 'number' ? med[0].toFixed(1) : ''}, q=${med[1].toFixed(2)}, chi_eff=${med[2].toFixed(2)}, D_L=${med[3].toFixed(0)} Mpc.">
    <text x="${pad}" y="14" fill="#7f8693" font-size="9" font-family="JetBrains Mono, monospace">POSTERIOR CORNER · diagonal: marginals · off-diagonal: 90% credible regions</text>
    ${panels}
  </svg>`;
  const sub = document.getElementById("extra2-subtitle");
  if (sub) sub.textContent = `${ev.name} · M=${med[0].toFixed(2)} M\u2609 · q=${med[1].toFixed(2)} · χ_eff=${med[2].toFixed(2)} · D_L=${med[3].toFixed(0)} Mpc.`;
  const body = document.getElementById("extra2-data-body");
  if (body) body.innerHTML = params.map((p, i) => `<tr><td>${p}</td><td>${typeof med[i] === 'number' && med[i] < 10 ? med[i].toFixed(2) : Math.round(med[i])}</td><td>${typeof lo[i] === 'number' && lo[i] < 10 ? lo[i].toFixed(2) : Math.round(lo[i])}</td><td>${typeof hi[i] === 'number' && hi[i] < 10 ? hi[i].toFixed(2) : Math.round(hi[i])}</td><td>${units[i]}</td></tr>`).join("");
}
...
function renderExtra3Panel() {
  ...
  if (!container) return;
  const data = [{"name":"f_low cutoff","units":"20→5Hz","down":-8.4,"up":12.6},{"name":"Network SNR","units":"+20%","down":-4.6,"up":5.2},{"name":"Mass posterior σ_M","units":"−25%","down":-3.4,"up":2.8},{"name":"Spin posterior σ_χ","units":"−25%","down":-1.8,"up":2.1},{"name":"Detector network","units":"+1 IFO","down":-2.6,"up":3.8},{"name":"Waveform systematics","units":"+1 PN","down":-1.4,"up":1.2}];
  ...
  const bars = data.map((d, i) => {
    ...
    const upBar = `<rect x="${xZero}" y="${y}" width="${ux.toFixed(1)}" height="${bh}" fill="#ff66aa" fill-opacity="0.72"/>`;
    ...
    const upText = `<text x="${xZero + ux + 4}" y="${y + bh / 2 + 4}" fill="#ff66aa" font-size="10" font-family="JetBrains Mono, monospace">+${d.up.toFixed(1)}%</text>`;
...
function renderExtra4Panel() {
  ...
  if (!container) return;
  const X = {"name":"SNR","values":[8,12,16,20,24,30]};
  const Y = {"name":"f_low (Hz)","values":[5,10,20,30,50]};
  const label = "σ_M / M (% relative)";
  const surface = (x, y) => Math.min(1, (1 / Math.max(1, x / 8)) * 0.6 + (y / 50) * 0.5);
  const palette = ["#1a2029", "#b34777", "#ff66aa", "#ff9cc3", "#ff3366"];
  const glyphs = [".", "·", "=", "▲", "■"];
